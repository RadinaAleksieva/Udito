...
5) Repository + docs prepared
- Initialized git repo in `/Users/mac/udito-app`.
- Added `README.md` with setup, env vars, routes, deploy steps.
- Committed full source and docs locally (commit: `55409c3`).
- Remote set to `https://github.com/RadinaAleksieva/Udito.git`.
- Push to GitHub NOT completed due to restricted network access.
...
1) Push repo to GitHub: `git -C /Users/mac/udito-app push -u origin main`.
2) Redeploy in Vercel so updated `WIX_APP_SECRET` takes effect.
3) Confirm/adjust `WIX_OAUTH_SCOPES` in Vercel to match Wix permissions.
4) Initialize DB: `POST /api/admin/init-db` with `Authorization: Bearer <ADMIN_SECRET>`.
5) Connect Wix: open `/overview` and click “Connect Wix” to complete OAuth.
6) Optional backfill: `POST /api/admin/backfill` to load historical orders.
7) Test export: `/api/audit/monthly?month=2024-01` (or custom date range).
